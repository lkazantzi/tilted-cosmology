...
the tilted ΛCDM bulk flow model (t-Λ) 
...
def q_param(z, a, b):
    #ratio = 1/(a+(b*(Dh*quad(lambda r:integrand(r), 0., z)[0]**3))) # then b should be equal to 4*10**(-11)
...
### luminosity distance of the tilted ΛCDM bulk flow model ###
def dl_param(z,a,b):
...
### the theoretical apparent magnitude of the tilted ΛCDM bulk flow model ###
...
def lnprior_eds(theta):
    a, b, Mcal = theta
### define priors 
    if 0.1 < a < 0.9 and 0. < b < 35. and 23. < Mcal < 24.0:
        return 0.0
    return -np.inf
...
initial = np.array([0.526, 3.707, 23.815])
...
with Pool() as pool:
    ...
    sampler_lcdm = emcee.EnsembleSampler(nwalkersl, ndiml, lnprob_eds, pool=pool)
    result_lcdm = sampler_lcdm.run_mcmc(posl, 2000, progress = True) # 2000 is the no of steps
...
np.savetxt("flatchains_lcdm2000 ",flat_samples_lcdm)

print("Number of independent samples is {}".format(len(flat_samples_lcdm)))

### The flatchains from the MCMC analysis discarded the burn-in steps 
flatchains_lcdm2000 = np.loadtxt("/home/kerky/anaconda3/test_q/resultsfromq(L)/q(l)_LCDM_1048/flatchains_ql_lcdm2000")
